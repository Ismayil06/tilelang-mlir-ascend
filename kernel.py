...
def bwd(
    ...
    def sparse_mqa_bwd_kernel(
        ...
        with T.Kernel(S * B * NH, is_npu=True) as (cid, _):
            ...
            T.copy(Delta[by, s_i, bz * block_H : (bz + 1) * block_H], Delta_shared[0:block_H])
            T.copy(Lse[by, s_i, bz * block_H : (bz + 1) * block_H], Lse_shared[0:block_H])
...
def run_one_bwd_case(
    ...
    # Compare
    torch.testing.assert_close(tl_dq_cpu.float(), ref_cpu["dq"].float(), rtol=5e-2, atol=5e-2)
